...
##
x = Double64.(5.1LinRange(-1, 1, 101))
y = Double64.(LinRange(-2.4, 3.8, 99))

g = LimitsGrid(z -> 1/(1 + z), x, y)

##
c₀ = distinguishable_colors(50, RGB(0, 0, 0))
c₁ = [c₀; repeat([:white], maximum(g.N) - length(c₀))]
c₂ = [c₀; :white]
c = c₁[1:maximum(g.N)-minimum(g.N)]

pN = heatmap(
	title = "Continued Fractions: x₀ + iy₀",
	yguide = "y₀",
	xguide = "x₀", 
	x, y, g.N',
	c = palette(c),
